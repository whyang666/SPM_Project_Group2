...
<script type="text/javascript">
    ...
    function deleteUser(flag){
        ...
        if(flag=="1"){
            msg = "您确定想要批量删除吗？";
            //selRow= $('#dg').datagrid("getSelections");//返回选中多行
        }else if(flag=="2"){
            msg = "您确定想要删除吗？";
            //selRow = $('#dg').datagrid("getSelected");//返回选中单行
        }

        $.messager.confirm('确认',msg,function(r){
            if (r){

                var selRow = $('#dg').datagrid("getSelections");//返回选中单行
                if(selRow.length==0){
    ...
    function uploadFile(){
        ...
        if(prefix=="xlsx"||prefix=="xls"){
            ...
            $('#fileUpload').form('submit',{
                url: "${ctx}/insertUserInfo.do",
                success: function(result){
                    $.messager.alert("提示",result);    //提示信息
    ...
    function addStudent(){
        ...
        $('#dialog').dialog('open').dialog('setTitle','学生信息导入');
...
<form id="ff" method="post">
	...
	<div style="text-align:center;padding:5px">
		...
		<input type="button" class="btn btn-default" style="margin-right:20px;" onclick="addStudent()" value="学生信息导入" />
...
<div id="dataList" style="display: none;" >
	...
	<div>
		...
		<div id="dialog" class="easyui-dialog"  style="padding:10px 20px;width: 700px" closed="true" buttons="#dlg-buttons" >
